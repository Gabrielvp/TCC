    ...
    public boolean insert(Orcamento orcamento) {
        ...
        try {
            PreparedStatement ps
                    = c.prepareStatement("INSERT INTO orcamento "
                            + "(data, qtdProduto, totalItem, cliente, descontoItem, aprovado, idPessoa, descontoOrcamento) "
                            + "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)");
            ps.setString(1, sdfD.format(orcamento.getData()));
            ps.setDouble(2, orcamento.getQtdProduto());
            ps.setDouble(3, orcamento.getTotal());
            ps.setString(4, orcamento.getPessoa().getNome());
            ps.setDouble(5, orcamento.getDescontoProduto());
            ps.setBoolean(6, orcamento.isAprovado());
            ps.setInt(7, orcamento.getPessoa().getIdPessoa());
            ps.setDouble(8, orcamento.getDescontoOrcamento());
